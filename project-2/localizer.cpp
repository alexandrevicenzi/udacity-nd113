...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  ...
  float belief_per_cell = 1.0 / (rows * columns);

  vector < vector <float> > beliefs (rows, vector <float> (columns, belief_per_cell));
  ...
  float blurring) 
{
  ...
  int columns = beliefs[0].size();

  vector < vector <float> > newGrid (rows, vector <float> (columns, 0.0));

    for (int i = 0; i < rows; ++i)
    {
      for (int j = 0; j < columns; ++j)
      {
        int new_i = mod(i + dy, rows);
        int new_j = mod(j + dx, columns);
  ...
  float p_miss) 
{
  ...
  vector< vector <float> > newBelifs (rows, vector <float> (columns, 0.0));
  ...
  for (int i = 0; i < rows; ++i)
  {
    for (int j = 0; j < columns; ++j)
    {
      if (grid[i][j] == color)
      {
        newBelifs[i][j] = beliefs[i][j] * p_hit;
      }
      else
      {
        newBelifs[i][j] = beliefs[i][j] * p_miss;
      }
  ...
  for (int i = 0; i < rows; ++i)
  {
    for (int j = 0; j < columns; ++j)
    {
      newBelifs[i][j] = newBelifs[i][j] / total;
    }
  }

  return normalize(newBelifs);
}
